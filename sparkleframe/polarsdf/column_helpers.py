...
def _make_runtime_arith_coercer(op: str):
    """Return a ``map_batches`` callback for cross-type arithmetic coercion at runtime.

    When both dtypes are unknown at build time, this callback inspects the actual
    dtypes of the ``(_arith_left, _arith_right)`` struct columns and applies Spark's
    mixed-type coercion rules.  The returned Series is a struct with the coerced pair.
    """

    def _coerce(s: pl.Series) -> pl.Series:
        struct_df = s.struct.unnest()
        left_s = struct_df.get_column("_arith_left")
        right_s = struct_df.get_column("_arith_right")
        ld = left_s.dtype
        rd = right_s.dtype
        left_num = _is_numeric_polars_dtype(ld)
        right_num = _is_numeric_polars_dtype(rd)
        left_str = _is_string_polars_dtype(ld)
        right_str = _is_string_polars_dtype(rd)

        if (left_num and right_str) or (left_str and right_num):
            coerced_l = left_s.cast(pl.Float64, strict=False)
            coerced_r = right_s.cast(pl.Float64, strict=False)
            return pl.DataFrame({"_arith_left": coerced_l, "_arith_right": coerced_r}).to_struct("_pair")
        ...
        if op in ("+", "-"):
            ...
            right_date = _is_date_polars_dtype(rd)
            if left_int and right_date:
                days = left_s.cast(pl.Int64, strict=False).cast(pl.Duration("ms"), strict=False) * 86_400_000
                coerced_r = right_s.cast(pl.Datetime("ms"), strict=False)
                return pl.DataFrame({"_arith_left": days, "_arith_right": coerced_r}).to_struct("_pair")
            if left_date and right_int:
                coerced_l = left_s.cast(pl.Datetime("ms"), strict=False)
                days = right_s.cast(pl.Int64, strict=False).cast(pl.Duration("ms"), strict=False) * 86_400_000
                return pl.DataFrame({"_arith_left": coerced_l, "_arith_right": days}).to_struct("_pair")

        _assert_arithmetic_series(left_s)
        _assert_arithmetic_series(right_s)
        target = _spark_numeric_widened_type(ld, rd)
        if target is not None:
            left_s = left_s.cast(target, strict=False)
            right_s = right_s.cast(target, strict=False)
        return pl.DataFrame({"_arith_left": left_s, "_arith_right": right_s}).to_struct("_pair")

    return _coerce


def _make_runtime_div_coercer():
    """Return a ``map_batches`` callback for cross-type division coercion at runtime."""
    ...
    def _coerce(s: pl.Series) -> pl.Series:
        ...
        if (_is_numeric_polars_dtype(ld) and _is_string_polars_dtype(rd)) or (
            _is_string_polars_dtype(ld) and _is_numeric_polars_dtype(rd)
        ):
            left_s = left_s.cast(pl.Float64, strict=False)
            right_s = right_s.cast(pl.Float64, strict=False)
        else:
            left_s = _validate_and_cast_float64_strict(left_s)
            right_s = _validate_and_cast_float64_strict(right_s)
        return pl.DataFrame({"_arith_left": left_s, "_arith_right": right_s}).to_struct("_pair")
